...
def crowded_comparison(s1, s2):#1 means s1 is better
    '''
...
class NSGAII:
    ...
    def crowding_distance_assignment(self, front):
        ...
        for obj_index in range(self.num_objectives):
            ...
            maxgap=front[len(front) - 1].objectives[obj_index]-front[0].objectives[obj_index]
            #for item in front:
                #print(item.objectives)
            if abs(maxgap)>0.000001:
                #print('gapmax is more than 0')
                for i in range(1, len(front) - 1):
                    front[i].distance += (front[i + 1].objectives[obj_index] - front[i - 1].objectives[obj_index])/maxgap